...
export const fetchAutomobiles = createAsyncThunk(GET, async () => {
  const response = await fetch('https://bookingapi-mstz.onrender.com/api/v1/automobiles');
  const data = await response.json();
  return { data };
});

export const createAutomobile = createAsyncThunk(ADD, async (automobileData) => {
  const response = await axios.post('https://bookingapi-mstz.onrender.com/api/v1/automobiles', { automobile: automobileData });
...
export const deleteAutomobile = createAsyncThunk(DELETE, async (id) => {
  await axios.delete(`https://bookingapi-mstz.onrender.com/api/v1/automobiles/${id}`);
